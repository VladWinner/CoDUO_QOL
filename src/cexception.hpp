...
/*
 *  ShowCrashMessageBox
 *      Displays a MessageBox with crash address and register information
 */
static void ShowCrashMessageBox(LPEXCEPTION_POINTERS pException, const wchar_t* logPath = nullptr)
{
    ...
    char log_path_ansi[MAX_PATH * 2];
    HMODULE module = GetModuleFromAddress(record.ExceptionAddress);
    ...
    // Convert log path to ANSI if provided
    if (logPath)
    {
        WideCharToMultiByte(CP_ACP, 0, logPath, -1, log_path_ansi, sizeof(log_path_ansi), NULL, NULL);
    }

    // Build the message
    int len = 0;
    len += sprintf(message + len, "========== APPLICATION CRASH ==========\n\n");

    // Crash location info
    len += sprintf(message + len, "Crash Address: 0x%p\n", address);
    len += sprintf(message + len, "Module: %s", FindModuleName(module, module_name, sizeof(module_name)));
    if (module) len += sprintf(message + len, " (+0x%X)", address - (uintptr_t)module);
    len += sprintf(message + len, "\n");

    // Full DLL path
    if (module && GetModuleFileNameA(module, module_name, sizeof(module_name)))
    {
        len += sprintf(message + len, "DLL Path: %s\n", module_name);
    }

    len += sprintf(message + len, "Exception: 0x%08X - %s\n",
        record.ExceptionCode, GetExceptionCodeString(record.ExceptionCode));

    // Additional exception info for access violations
    if (record.ExceptionCode == EXCEPTION_ACCESS_VIOLATION || record.ExceptionCode == EXCEPTION_IN_PAGE_ERROR)
    {
        auto rw = (DWORD)record.ExceptionInformation[0];
        auto addr = (ULONG_PTR)record.ExceptionInformation[1];
        len += sprintf(message + len, "Access Type: %s address 0x%p\n",
            rw == 0 ? "Reading" : rw == 1 ? "Writing" : rw == 8 ? "DEP violation at" : "Unknown access to",
            addr);
    }
    ...
    {
        len += sprintf(message + len, "========== REGISTERS ==========\n");
#if !_M_X64
        len += sprintf(message + len, "EAX: 0x%08X  EBX: 0x%08X\n", context.Eax, context.Ebx);
        len += sprintf(message + len, "ECX: 0x%08X  EDX: 0x%08X\n", context.Ecx, context.Edx);
        len += sprintf(message + len, "ESI: 0x%08X  EDI: 0x%08X\n", context.Esi, context.Edi);
#else
        len += sprintf(message + len, "RAX: 0x%016llX  RBX: 0x%016llX\n", context.Rax, context.Rbx);
        len += sprintf(message + len, "RCX: 0x%016llX  RDX: 0x%016llX\n", context.Rcx, context.Rdx);
        len += sprintf(message + len, "RSI: 0x%016llX  RDI: 0x%016llX\n", context.Rsi, context.Rdi);
        len += sprintf(message + len, "R8:  0x%016llX  R9:  0x%016llX\n", context.R8, context.R9);
        len += sprintf(message + len, "R10: 0x%016llX  R11: 0x%016llX\n", context.R10, context.R11);
        len += sprintf(message + len, "R12: 0x%016llX  R13: 0x%016llX\n", context.R12, context.R13);
        len += sprintf(message + len, "R14: 0x%016llX  R15: 0x%016llX\n", context.R14, context.R15);
#endif
    }
    ...
    {
#if !_M_X64
        len += sprintf(message + len, "EBP: 0x%08X  ESP: 0x%08X\n", context.Ebp, context.Esp);
        len += sprintf(message + len, "EIP: 0x%08X  EFL: 0x%08X\n", context.Eip, context.EFlags);
#else
        len += sprintf(message + len, "RBP: 0x%016llX  RSP: 0x%016llX\n", context.Rbp, context.Rsp);
        len += sprintf(message + len, "RIP: 0x%016llX  EFL: 0x%08X\n", context.Rip, context.EFlags);
#endif
    }
    ...
    {
        len += sprintf(message + len, "\n========== CRASH DUMP ==========\n");
        len += sprintf(message + len, "Log saved to:\n%s", log_path_ansi);
    }

    MessageBoxA(NULL, message, "Application Crash", MB_OK | MB_ICONERROR | MB_SYSTEMMODAL | MB_TOPMOST);
}
...
LONG WINAPI CustomUnhandledExceptionFilter(LPEXCEPTION_POINTERS ExceptionInfo)
{
    ...
    // step 2: write log
    swprintf_s(logfilename, L"%s\\%s\\%s.%s.log", modulename, L"CrashDumps", modulenameptr, timestamp);
    hFile = CreateFileW(logfilename, GENERIC_WRITE, FILE_SHARE_WRITE, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
    ...
    // STEP 3: Show MessageBox with crash info (after writing log so we can show the path)
    ShowCrashMessageBox(ExceptionInfo, logfilename);
